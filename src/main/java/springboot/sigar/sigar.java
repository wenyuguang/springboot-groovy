...
public class sigar {
	...
	static {
		String dllName = "/lib/";
		if(isWin32()) {
			dllName = dllName.concat("sigar-x86-winnt.dll");
		}else if(isWin64()) {
			dllName = dllName.concat("sigar-amd64-winnt.dll");
		}else if(isLinux32()) {
			dllName = dllName.concat("libsigar-x86-linux.so");
		}else if(isLinux64()) {
			dllName = dllName.concat("libsigar-amd64-linux.so");
		}
		try {
			...
			System.out.println("filePath sigar-amd64-winnt.dll is " + filePath);
			...
			log.error("load jni dll error : {}", e.toString());
		}
	}
    ...
    private static boolean isWin32() {
		return OS.contains(WIN) && ARCH.contains("32");
	}
	
	private static boolean isLinux32() {
		return OS.contains(LINUX) && ARCH.contains("32");
	}
	
	private static boolean isLinux64() {
		return OS.contains(LINUX) && ARCH.contains("64");
	}
